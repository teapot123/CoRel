...
char train_file[MAX_STRING], output_file[MAX_STRING], output_tmp_file[MAX_STRING], kappa_file[MAX_STRING], topic_file[MAX_STRING], output_tmp_file_1[MAX_STRING];
char topic_output[MAX_STRING];
char context_output[MAX_STRING];
char doc_output[MAX_STRING];
char load_emb_file[MAX_STRING], load_temb_file[MAX_STRING], load_emb_cap_file[MAX_STRING];
...
real *syn0, *syn1neg, *syn1doc, *expTable, *t0_embeddings, *cap0;
real *kappa;
real L = 0.0;
clock_t start;
...
int topic_words = 0;
int cat_group_num = 0;
int *topic_index, *parent_topic_index, *category_depth;
int *category_length, *category_start, *topic_category;
// int *topic_start_end;
...
int kappaRankingComparator(const void *a, const void *b) { // small -> large
  real aSmall0, bSmall0;
  if (topic_pivot_idx < vocab_size) {
    aSmall0 = (kappa[(*(int *) a) % vocab_size] - kappa[topic_pivot_idx]) * (kappa[(*(int *) a) % vocab_size] - kappa[topic_pivot_idx]);
    bSmall0 = (kappa[(*(int *) b) % vocab_size] - kappa[topic_pivot_idx]) * (kappa[(*(int *) b) % vocab_size] - kappa[topic_pivot_idx]);  
  } 
  else {
    aSmall0 = (kappa[(*(int *) a) % vocab_size] - cap0[topic_pivot_idx - vocab_size]) * (kappa[(*(int *) a) % vocab_size] - cap0[topic_pivot_idx - vocab_size]);
    bSmall0 = (kappa[(*(int *) b) % vocab_size] - cap0[topic_pivot_idx - vocab_size]) * (kappa[(*(int *) b) % vocab_size] - cap0[topic_pivot_idx - vocab_size]);  
  }
...
void InitNet() {
  ...
  if (load_emb) {
    ...
    printf("initialization: %d %f\n", 0, syn0[89 * layer1_size]);
  }
  if (load_emb_with_t) {
    char * temb_file = (char *) calloc(MAX_STRING, sizeof(char));
    strcpy(temb_file, load_temb_file);
    printf("loading topic embedding from %s\n", temb_file);
    if (access(temb_file, R_OK) == -1) {
      printf("%s does not exist\n",temb_file);
      return;
    }
    int t0_size_tmp, word_dim;
    char * current_word = (char *) calloc(MAX_STRING, sizeof(char));
    FILE *fp = fopen(temb_file, "r");
    fscanf(fp, "%d", &t0_size_tmp);
    fscanf(fp, "%d", &word_dim);
    a = posix_memalign((void **) &t0_embeddings, 128, (long long) t0_size_tmp * word_dim * sizeof(real));
    if (t0_embeddings == NULL) {
      printf("Memory allocation failed\n");
      exit(1);
    }
    for (a = 0; a < t0_size_tmp; a++) {
      fscanf(fp, "%s", current_word);
      char * t = (char *) calloc(MAX_STRING, sizeof(char));
      sprintf(t, "_t%d_", a);
      if(strcmp(current_word, t) != 0) {
        printf("topic name is not aligned! Line %lld should be %s\n",a+2,t);
        exit(0);
      }
      for (b = 0; b < layer1_size; b++)
        fscanf(fp, "%f", &t0_embeddings[a * layer1_size + b]);
    }
    fclose(fp);
    //load kappa of expanded topics
    char * emb_cap_file = (char *) calloc(MAX_STRING, sizeof(char));
    strcpy(emb_cap_file, load_emb_cap_file);
    printf("loading topic kappa from %s\n", emb_cap_file);
    if (access(emb_cap_file, R_OK) == -1) {
      printf("%s does not exist\n",emb_cap_file);
      return;
    }
    fp = fopen(emb_cap_file, "r");
    fscanf(fp, "%d", &t0_size_tmp);
    a = posix_memalign((void **) &cap0, 128, (long long) t0_size_tmp * sizeof(real));
    if (cap0 == NULL) {
      printf("Memory allocation failed\n");
      exit(1);
    }
  ...
  if (with_regularization) {
    ...
    for (a = 0; a < 2; a++) {
      ...
      while ((read = getline(&line, &len, fp)) != -1) {
        ...
        for (int i = 0; i <= read; i++) {
          ...
          if (line[i] == '\t') {
            ...
            if ((vocab_idx = SearchVocab(tmp_word)) != -1) {
              int flag = 0;
              if (a != 0) {
                for (int j = 0; j < topic_words; j++) {
                  if (topic_index[j] == vocab_idx) {
                    parent_topic_index[cat_group_num] = j;
                    category_depth[cat_group_num] = category_depth[topic_category[j]] + 1;
                    if (category_depth[cat_group_num] > max_depth) max_depth = category_depth[cat_group_num];
                    flag = 1;
                    break;
                  }
                }
                if (flag == 0) {
                  printf("The root node in the %dth row is wrong!\n", cat_group_num);
                  exit(1);
                }                
              } 
            } else if(tmp_word[0] == 'R' && tmp_word[1] == 'O' && tmp_word[2] == 'O' && tmp_word[3] == 'T' ) {
              if (a!=0) {
                parent_topic_index[cat_group_num] = -1;
                category_depth[cat_group_num] = 0;
              } 
            } else {
          ...
          if (line[i] == ' ' || i == read) {
            ...
            if ((vocab_idx = SearchVocab(tmp_word)) != -1) {
              if (a != 0) {
                ...
                topic_category[iidx++] = cat_group_num;
                topics++;
              } else {
                topic_words++;
              }
            } else if (tmp_word[0] == '_' && tmp_word[1] == 't'){
              if (a != 0) {
                strncpy(tmp_word, line + st + 2, i - st);
                topic_index[iidx] = vocab_size + atoi(tmp_word);
                topic_category[iidx++] = cat_group_num;
                topics++;
              } else {
                topic_words++;
        ...
        // if (a == 0) topics++;
        if (a != 0) {
          category_length[cat_group_num] = topics;
          if (cat_group_num == 0) category_start[cat_group_num] = 0;
          else category_start[cat_group_num] = category_start[cat_group_num - 1] + category_length[cat_group_num - 1];
        }        
        cat_group_num++;
        // if (a != 0) {
      ...
      if (a == 0) {
        ...
        parent_topic_index = calloc(cat_group_num, sizeof(int));
        category_depth = calloc(cat_group_num, sizeof(int));
        category_length = calloc(cat_group_num, sizeof(int));
        category_start = calloc(cat_group_num, sizeof(int));
        topic_index = calloc(topic_words, sizeof(int));
        topic_category = calloc(topic_words, sizeof(int));
      }
      if (a != 0) {
        printf("number of category groups = %d\n", cat_group_num);
        printf("number of all topics = %d\n", topic_words);
        topics = topic_words; //align with later code
        // for (int i = 0; i < cat_group_num; i++) {
        //   printf("%d ",parent_topic_index[i]);
        // }
        // printf("\n");
        // for (int i = 0; i < cat_group_num; i++) {
        //   printf("%d ",category_depth[i]);
        // }
        // printf("\n");
        // for (int i = 0; i < cat_group_num; i++) {
        //   printf("%d ",category_length[i]);
        // }
        // printf("\n");
        // for (int i = 0; i < cat_group_num; i++) {
        //   printf("%d ",category_start[i]);
        // }
        // printf("\n");
        // for (int i = 0; i < topic_words; i++) {
        //   printf("%d ",topic_index[i]);
        // }
        // printf("\n");
        // for (int i = 0; i < topic_words; i++) {
        //   printf("%d ",topic_category[i]);
        // }
        // printf("\n");
      }
      fclose(fp);
    ...
    // exit(1);
    for (a = 0; a < topics; a++) {
      int words_in_topic = 1;//topic_start_end[a * 2 + 1] - topic_start_end[a * 2];
      if (a == 0) {
...
real regularize_in_children(int a, int b, int current_tid, int max_depth, int cat_id, real *wi, real *exp_rij, real *delta_wik, real *delta_tjk, real L, real scaling) {
  if (category_depth[cat_id] < max_depth) {
    for (int t = 0; t < cat_group_num; t++) {
      if (parent_topic_index[t] == current_tid) {
        int n = category_length[t];
        wi[a * num_per_topic + b] = 0;
        real sum_eij=0.0;
        int overflow = 0, underflow = 0;
        real max_rij = 0.0, min_rij = 0.0;
        int word_index = fix_seed_rankings[a * vocab_size + b] % vocab_size;
        for (int d = category_start[t]; d < category_start[t] + category_length[t]; d++) {
          real rij = 0.0;
          for (int c = 0; c < layer1_size; c++) {
            rij += syn0[word_index * layer1_size + c] * t_embeddings[d * layer1_size + c];
          }
          // if (a == 0) printf("(child) topic index: %d, word: %s, rij: %f, syn0: %f, t_emb: %f\n", d, vocab[word_index].word, rij, syn0[word_index * layer1_size], t_embeddings[d * layer1_size]);             
          // dealing with overflow or underflow rij
          if (rij < -20) {
            underflow = 1;
            // if(a == 0) printf("detect underflow!\n");
            if (rij < min_rij) min_rij = rij;
          }
          else if (rij > 20) {
            overflow = 1;
            // if(a == 0) printf("detect overflow!\n");
            if (rij > max_rij) max_rij = rij;
          }
          real eij = exp(rij);
        ...
        L += -sum_eij/n + log(wi[a * num_per_topic + b]) + log(1.0/n);
        for (int c = 0; c < layer1_size; c++) {
        ...
        for (int d = category_start[t]; d < category_start[t] + category_length[t]; d++) {
          // if (a == 0) {
          //   printf("ratio of topic %d: %f\n", d, exp_rij[(a * num_per_topic + b) * topics + d] / wi[a * num_per_topic + b]);
          // }
          for (int c = 0; c < layer1_size; c++) {
        ...
        int a_start = category_start[t], a_end = category_start[t] + category_length[t];
        for (int d = a_start; d < a_end; d++) {
          L = regularize_in_children(a, b, topic_index[d], max_depth, topic_category[d], wi, exp_rij, delta_wik, delta_tjk, L, scaling);
        }
      }
    }
  }
...
real regularize_topic_in_children(int a, int current_tid, int max_depth, int cat_id, real *wi, real *exp_rij, real *delta_wik, real *delta_tjk, real L, real scaling) {
  if (category_depth[cat_id] < max_depth) {
    for (int t = 0; t < cat_group_num; t++) {
      if (parent_topic_index[t] == current_tid) {
        int n = category_length[t];
        wi[a * num_per_topic] = 0;
        real sum_eij=0.0;
        int overflow = 0, underflow = 0;
        real max_rij = 0.0, min_rij = 0.0;
        for (int d = category_start[t]; d < category_start[t] + category_length[t]; d++) {
          real rij = 0.0;
          for (int c = 0; c < layer1_size; c++) {
            rij += t_embeddings[a * layer1_size + c] * t_embeddings[d * layer1_size + c];
          }
          // if (a == 0) printf("(child) topic index: %d, word: %s, rij: %f, syn0: %f, t_emb: %f\n", d, vocab[word_index].word, rij, syn0[word_index * layer1_size], t_embeddings[d * layer1_size]);             
          // dealing with overflow or underflow rij
          if (rij < -20) {
          ...
          real eij = exp(rij);
          exp_rij[(a * num_per_topic ) * topics + d] = eij;
          wi[a * num_per_topic ] += eij;
          // if (a == d) rci = rij;
          sum_eij += rij;
        }
        ...
        L += -sum_eij/n + log(wi[a * num_per_topic ]) + log(1.0/n);
        for (int c = 0; c < layer1_size; c++) {
          int wik_index = a * layer1_size + c;
          delta_tjk[wik_index] = 0.0;
          for (int d = category_start[t]; d < category_start[t] + category_length[t]; d++) {
            delta_tjk[wik_index] += t_embeddings[d * layer1_size + c] *
                                    (exp_rij[(a * num_per_topic ) * topics + d] /
                                     wi[a * num_per_topic ] - 1.0/n);
            int tjk_index = d * layer1_size + c;
            delta_tjk[tjk_index] += t_embeddings[a * layer1_size + c] *
                                  (exp_rij[(a * num_per_topic ) * topics + d] /
                                   wi[a * num_per_topic ] - 1.0/n);
          }
        }
        ...
        //update center word embedding
        for (int c = 0; c < layer1_size; c++) {
          t_embeddings[a * layer1_size + c] +=
              scaling * delta_tjk[a * layer1_size + c];
        }
        ...
        // int a_start = category_start[t], a_end = category_start[t] + category_length[t];
        // for (int d = a_start; d < a_end; d++) {
        //   L = regularize_topic_in_children(a, topic_index[d], max_depth, topic_category[d], wi, exp_rij, delta_wik, delta_tjk, L, scaling);
        // }
      }
    }
  }
...
void *TrainModelThread(void *id) {
  // printf("start new training thread!\n");
  ...
  while (1) {
    if (word_count - last_word_count > 10000) {
      ...
      if ((debug_mode > 1)) {
        now = clock();
        printf("%cAlpha: %f  Loss: %f  Progress: %.2f%%  Words/thread/sec: %.2fk  ", 13, alpha, L,
    ...
    if (with_regularization && !is_pretrain) {
      ...
      if (word_counter == 0) {
        ...
        //clear t_embedding update matrix
        ...
        for (a = 0; a < topics; a++)
          for (b = 0; b < num_per_topic; b++) {
            // if(a == 2)
            //   printf("current topic index: %d\n", a);
            int word_index = fix_seed_rankings[a * vocab_size + b] % vocab_size;
            wi[a * num_per_topic + b] = 0;
            real rci = 0.0;
            int cat_id = topic_category[a];
            int fixed_cat_id = cat_id;
            int p_index = parent_topic_index[cat_id];
            // discriminate over current level
            wi[a * num_per_topic + b] = 0;
            int overflow = 0, underflow = 0;
            real max_rij = 0.0, min_rij = 0.0;
            real L1 = 0.0;
            for (int i = 0; i < cat_group_num; i++) {
              if (category_depth[i] != category_depth[cat_id]) {
                continue;
              }             
              for (d = category_start[i]; d < category_start[i] + category_length[i]; d++) {
                real rij = 0.0;
                for (c = 0; c < layer1_size; c++) {
                  rij += syn0[word_index * layer1_size + c] * t_embeddings[d * layer1_size + c];
                }
                ...
                // if(a == 0) printf("(parent) topic index: %d, word: %s, rij: %f, syn0: %f, t_emb: %f\n", d, vocab[word_index].word, rij, syn0[word_index * layer1_size], t_embeddings[d * layer1_size]);             
                real eij = exp(rij);
                exp_rij[(a * num_per_topic + b) * topics + d] = eij;
                wi[a * num_per_topic + b] += eij;
                if (a == d) rci = rij;
              ...
              // if (underflow) {
              //   wi[a * num_per_topic + b] = 0;
              //   for (d = category_start[cat_id]; d < category_start[cat_id] + category_length[cat_id]; d++) {
              //     real rij = 0.0;
              //     for (c = 0; c < layer1_size; c++) {
              //       rij += syn0[word_index * layer1_size + c] * t_embeddings[d * layer1_size + c];
              //     }
              //     // if(a == 0) printf("after underflow processing: (parent) topic index: %d, word: %s, rij: %f, syn0: %f, t_emb: %f\n", d, vocab[word_index].word, rij, syn0[word_index * layer1_size], t_embeddings[d * layer1_size]);             
              //     real eij = exp(rij - min_rij);
              //     exp_rij[(a * num_per_topic + b) * topics + d] = eij;
              //     wi[a * num_per_topic + b] += eij;
              //     if (a == d) rci = rij;
              ...
              //       rij += syn0[word_index * layer1_size + c] * t_embeddings[d * layer1_size + c];
              //     }
              //     // if(a == 0) printf("after overflow processing: (parent) topic index: %d, word: %s, rij: %f, syn0: %f, t_emb: %f\n", d, vocab[word_index].word, rij, syn0[word_index * layer1_size], t_embeddings[d * layer1_size]);             
              //     real eij = exp(rij - max_rij);
              //     exp_rij[(a * num_per_topic + b) * topics + d] = eij;
              //     wi[a * num_per_topic + b] += eij;
              //     if (a == d) rci = rij;
            ...
            // if (L < 0) 
            L1 += -rci + log(wi[a * num_per_topic + b]);
            // if (L1 < 0.001)
            //   continue;
              // printf("topic a: %s %f\n", vocab[topic_index[a]].word, L1);
            for (c = 0; c < layer1_size; c++) {
              int wik_index = (a * num_per_topic + b) * layer1_size + c;
              delta_wik[wik_index] = 0.0;
              for (int i = 0; i < cat_group_num; i++) {
                if (category_depth[i] != category_depth[cat_id]) continue;
                for (d = category_start[i]; d < category_start[i] + category_length[i]; d++) {
                  delta_wik[wik_index] += t_embeddings[d * layer1_size + c] *
                                          (exp_rij[(a * num_per_topic + b) * topics + d] /
            ...
            // printf("t[%d]: %f\n", a, t_embeddings[a * layer1_size]);


            
            // child levels: uniform distribution
            cat_id = fixed_cat_id;
            // L = regularize_in_children(a, b, a, max_depth, cat_id, wi, exp_rij, delta_wik, delta_tjk, L, scaling);             
            // if (b==0)
            //   L = regularize_topic_in_children(a, a, max_depth, cat_id, wi, exp_rij, delta_wik, delta_tjk, L, scaling);             
          }
      }
    }
    ...
    for (a = b; a < window * 2 + 1 - b; a++)
      if (a != window) {
        ...
        if (negative > 0) {
          ...
          // if (last_word == 89) printf("after local center%d %f\n", 0, syn0[last_word * layer1_size]);
    ...
    // if (word == 89) printf("after local context%d %f\n", 0, syn0[word * layer1_size]);
    if (with_global) {
      for (c = 0; c < layer1_size; c++) neu1e[c] = 0;
      if (negative > 0) {
        real kappa_update = 0.0;
      ...
      // if (word == 89) printf("after global%d %f\n", 0, syn0[word * layer1_size]);
    }
    ...
    //               scaling * delta_wik[(a * num_per_topic + b) * layer1_size + c];
    //         }
    //         // if (word_index == 89) printf("after reg%d %f\n", 0, syn0[word_index * layer1_size]);
    //       }
    //     for (d = 0; d < topics; d++)
    //       for (c = 0; c < layer1_size; c++) {
    //         t_embeddings[d * layer1_size + c] += scaling * delta_tjk[d * layer1_size + c];
    //         if ( c == 0 && d == 0 ) printf("topic index: %d, delta_tjk: %f, t_embeddings: %f\n", d, delta_tjk[d * layer1_size], t_embeddings[d * layer1_size]);
    //       }
    //   }
    // }
...
void generate_filename(char* origin_filename, int iter_num) {
  char base_string[MAX_STRING];
  int filename_length = 0;
  for (int j=0; j<MAX_STRING ;j++) {
    if(origin_filename[j] == 0) {
      filename_length = j;
      break;
    }
  }
  for (int j=0; j<filename_length-4; j++) base_string[j]=origin_filename[j];
  base_string[filename_length-4]=0;
  // printf("base_string: %s\n", base_string);
  strcpy(output_tmp_file_1, base_string);
  // printf("output_tmp_file_1: %s\n", output_tmp_file_1);
  strcat(output_tmp_file_1, "_");
  // printf("output_tmp_file_1: %s\n", output_tmp_file_1);
  char * t = (char *) calloc(MAX_STRING, sizeof(char));
  sprintf(t, "%d", iter_num);
  strcat(output_tmp_file_1, t);
  // printf("output_tmp_file_1: %s\n", output_tmp_file_1);
  strcat(output_tmp_file_1, ".txt");
  printf("output_tmp_file_1: %s\n", output_tmp_file_1);
}
...
void TrainModel() {
  ...
  if (with_regularization) {
    for (a = 0; a < topics; a++) {
      if (topic_index[a] < vocab_size) 
        for (b = 0; b < layer1_size; b++) {
          t_embeddings[a * layer1_size + b] = syn0[topic_index[a] * layer1_size + b];
        }
      else {
        for (b = 0; b < layer1_size; b++) {
          t_embeddings[a * layer1_size + b] = t0_embeddings[(topic_index[a] - vocab_size) * layer1_size + b];
        }
      }
    }
    ...
    for (iter_count = pretrain_iters; iter_count < iter; iter_count++) {
      ...
      if (fix_seed) {
        ...
        for (a = 0; a < topics; a++) {
          int ind = 0;
          for (b = 0; b < num_per_topic; b++) {
            int keyword = rankings[a * vocab_size + ind] % vocab_size;
            while (1) {              
              int flag = 1;
              for (c = 0; c < topics; c++) {
                if (c == a) continue;
                if (topic_index[c] == keyword) {
                  printf("keyword of topic %d:%d is the same as topic name %d:%d\n", a, keyword, c, topic_index[c]);
                  flag = 0;
                  ind++;
                  keyword = rankings[a * vocab_size + ind] % vocab_size;
                  break;
                }
              }
              if (flag) break;
            ...
            ind++;
            fix_seed_rankings[a * vocab_size + b] = keyword;
          }
      ...
      for (a = 0; a < topics; a++) {
        ...
        for (b = 0; b < num_per_topic; b++) {
          ...
          // printf("%d,",  kappa_rankings[rankings[a * vocab_size + b]]);
          // printf("%f ", kappa[fix_seed_rankings[a * vocab_size + b] % vocab_size]);
        }
        // printf("top word in kapparank: %d, kappa: %f \n", rankings1[a*vocab_size] % vocab_size, kappa[rankings1[a*vocab_size] % vocab_size]);
      ...
      printf("start printing word vectors!\n");
      //print word vectors during training
      generate_filename(output_file, iter_count - pretrain_iters);
      fmid = fopen(output_tmp_file_1, "wb");
      fprintf(fmid, "%lld %lld\n", vocab_size, layer1_size);
      for (a = 0; a < vocab_size; a++) {
        fprintf(fmid, "%s ", vocab[a].word);
        if (binary)
          for (b = 0; b < layer1_size; b++) {
            fwrite(&syn0[a * layer1_size + b], sizeof(real), 1, fmid);
          }
        else
          for (b = 0; b < layer1_size; b++) {
            fprintf(fmid, "%lf ", syn0[a * layer1_size + b]);
          }
        fprintf(fmid, "\n");
      }
      if (context_output[0] != 0) {
        generate_filename(context_output, iter_count - pretrain_iters);
        FILE* fa = fopen(output_tmp_file_1, "wb");
        fprintf(fa, "%lld %lld\n", vocab_size, layer1_size);
        for (a = 0; a < vocab_size; a++) {
      ...
      if (with_kappa) {
        generate_filename(kappa_file, iter_count - pretrain_iters);
        fmid = fopen(kappa_file, "wb");
        fprintf(fmid, "%lld\n", vocab_size);
        for (a = 0; a < vocab_size; a++) {
          fprintf(fmid, "%s ", vocab[a].word);
          fprintf(fmid, "%lf\n", kappa[a]);
      ...
      if (with_regularization) {
        generate_filename(topic_output, iter_count - pretrain_iters);
        fmid = fopen(output_tmp_file_1, "wb");
        fprintf(fmid, "%d\n", topics);
        for (a = 0; a < topics; a++) {
          // for (int i = topic_start_end[a * 2]; i < topic_start_end[a * 2 + 1]; i++) {
          ...
          // fprintf(fp, "\t");
          for (b = 0; b < layer1_size; b++) {
            fprintf(fmid, " %lf", t_embeddings[a * layer1_size + b]);
          }
          fprintf(fmid, "\n");
        }
        fclose(fmid);
      }
      printf("word vectors printed!\n");
...
int main(int argc, char **argv) {
  ...
  if ((i = ArgPos((char *) "-load_emb_with_t", argc, argv)) > 0) load_emb_with_t = atoi(argv[i + 1]);
  if ((i = ArgPos((char *) "-load_emb_file", argc, argv)) > 0) strcpy(load_emb_file, argv[i + 1]);
  if ((i = ArgPos((char *) "-load_emb_cap_file", argc, argv)) > 0) strcpy(load_emb_cap_file, argv[i + 1]);
  if ((i = ArgPos((char *) "-load_temb_file", argc, argv)) > 0) strcpy(load_temb_file, argv[i + 1]);
