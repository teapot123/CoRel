...
MODEL=${MODEL:- ${MODELS_DIR}/$1}
if [ -d "default_data" ]; then
    DATA_DIR=${DATA_DIR:- default_data}
else
    DATA_DIR=${DATA_DIR:- data}
fi
TEXT_TO_SEG=${TEXT_TO_SEG:- ../$1/$2}
OUTPUTPATH="$1"
...
python process_segmentation.py --multi $HIGHLIGHT_MULTI --single $HIGHLIGHT_SINGLE --output $OUTPUTPATH --mode whole
